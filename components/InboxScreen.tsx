...
export default function InboxScreen({ tasks, onScheduleToday, onDelete }: Props) {
  if (tasks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[calc(100dvh-var(--tab-bar-height))] gap-4 text-center p-8">
  ...
  return (
    <div className="flex flex-col gap-3 p-4 pb-6">
      <h1 className="text-xl font-bold pt-4 text-white">
        Вхідні{" "}
        <span className="text-sky-400 font-normal">({tasks.length})</span>
      </h1>
